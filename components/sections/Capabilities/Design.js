...
const Design = ({ title, design }) => {
  const { style, branding, ux, tools } = design;

  return (
    <Wrapper class="design" customClassName="flex items-center">
      <MainTitle title={title} />
      <div className="grid grid-cols-[1fr_1fr] gap-10 justify-between lg:grid-cols-1">
        <div className="relative">
          <SectionTitle>{design.title.toUpperCase()}</SectionTitle>
          <AnimatedVerticalBar
          ...
          <AnimatedHorizontalBar
            classes="absolute left-0 lg:top-[9rem] md:top-[7rem] h-[2rem] bg-[--accent] hidden lg:block"
            extent="100vw"
          />
          <AnimatedHorizontalBar
            classes="absolute left-0 lg:top-[9rem] md:top-[7rem] h-[2rem] bg-[--metal-20] hidden lg:block"
            extent="100vw"
            delay={1}
          />
        </div>
        <ul className="flex flex-col gap-[10rem] md:gap-[4rem] md:py-[3rem] z-10">
          {[style, branding, ux, tools].map((item, index) => {
            return (
              <motion.li
                initial={{
                  opacity: 0,
